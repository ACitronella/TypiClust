...
class ActiveLearning:
    ...
    def sample_from_uSet(self, clf_model, lSet, uSet, trainDataset, supportingModels=None, **kwargs):
        ...
        indices_table = trainDataset.indices_table
        patient_count = pd.Series([trainDataset.get_patient_code_and_frame_from_idx(idx)["patient_code"] for idx in lSet]).value_counts().to_dict()
        listof_patient = [(p_code, patient_count.get(p_code, 0)) for p_code in self.dataset_info["patient_code"].unique()]
        listof_patient = sorted(listof_patient, key=lambda x:x[1])
        print(listof_patient)
        p_code_tobe_selected = listof_patient[0][0]
        p_idx = self.dataset_info[self.dataset_info["patient_code"] == p_code_tobe_selected].index
        uSet = []
        for idx in p_idx:

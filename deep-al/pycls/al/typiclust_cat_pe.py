...
class TypiClustCatPE:
    ...
    def __init__(self, cfg, lSet, uSet, budgetSize, is_scan=False, dataset_info=None):
        ...
        self.pe = CatPositionalEncoding(d_model=512) # d_model is probably embedding size
        self.indices_table = (self.dataset_info["frames"].cumsum() - self.dataset_info["frames"]).values # collect start index of each eye self.indices_table = np.concatenate([self.indices_table, [self.dataset_info["frames"].sum()]]) # for last file
    ...
    def init_features_and_clusters(self, is_scan):
        ...
        if is_scan:
            ...
            self.new_features = torch.zeros((self.features.shape[0], self.features.shape[1]*2))
            for start_idx, end_idx in zip(self.indices_table, self.indices_table[1:]):
                self.new_features[start_idx:end_idx] = self.pe(self.features[start_idx:end_idx])
            self.features = self.new_features.numpy()

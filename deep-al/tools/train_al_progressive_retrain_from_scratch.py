...
from pycls.al.ActiveLearningForLeastFramePatient import ActiveLearningForLeastFramePatient as ActiveLearning
    ...
    while count_current_epochs < num_epochs:
        ...
        if ((how_best == "min" and cur_metric < cur_best_metric) or (how_best == "max" and cur_metric > cur_best_metric)):
            ...
            if use_patient_and_save_best:
                count_current_epochs = 0
        train_loader.num_workers = 8 # because of cache in the Custom dataset class
...
def get_embedding_path(cfg, fold_idx):
    return cfg.ACTIVE_LEARNING.EMBEDDING_PATH.format(fold_idx=fold_idx)
def get_pretrained_path(cfg, fold_idx, n_train):
    return cfg.PRETRAINED_WEIGHTS_EACH_PATIENT.format(fold_idx=fold_idx, n_train=n_train)
...
def main(cfg):
    ...
    for fold_idx in range(FOLDS):
        ...
        embedding_path = get_embedding_path(cfg, fold_idx)
        msg = "Dataset {} Loaded Sucessfully. Total Train Size: {}\n".format(cfg.DATASET.NAME, train_size,)
        print(msg);logger.info(msg)
        exp_patient_dir = os.path.join(cfg.EXP_DIR, f"fold{fold_idx}")
        ...
        # print(train_patient_code, unlabel_patient_code, all_patient_code)
         
        lSet_path, uSet_path, valSet_path = data_obj.makeLUNSetsByPatients(train_patient_code, unlabel_patient_code, all_data, exp_patient_dir)
        ...
        lSet, uSet, vSet = data_obj.loadPartitions(lSet_path, uSet_path, valSet_path)

        relevent_set = np.concatenate([lSet, uSet])
        test_related_set = np.setdiff1d(np.arange(len(all_data), dtype="int32"), relevent_set)
        assert not np.any(np.isin(relevent_set, test_related_set))
        ...
        model.load_state_dict(init_model_state_dict, strict=True); model.train()
        ...
        train_dataloader_wrapper = functools.partial(DataLoader, batch_size=cfg.TRAIN.BATCH_SIZE, num_workers=0, shuffle=True, pin_memory=True)
        ...
        msg = "Labeled Set: {} ({})\nUnlabeled Set: {} ({})\nValidation Set: {} ({})".format(len(lSet), train_patient_code, len(uSet), unlabel_patient_code, len(vSet), val_patient_code)
        print(msg); logger.info(msg)
        
        best_pretrained_model_save_path = get_pretrained_path(cfg, fold_idx, len(train_dataset))
        ...
        if not cfg.PROGRESSIVE_ADD_IN_LSET:
            # Reset lSet
            lSet = np.asarray([])
            msg = "drop all patient, to be added from uSet as PROGRESSIVELY_ADD_IN_LSET is set to False"
            print(msg);logger.info(msg)
        ...
        for cur_episode in range(0, cfg.ACTIVE_LEARNING.MAX_ITER):
            ...
            al_obj = ActiveLearning(data_obj, cfg, cur_episode+1, dataset_info, embedding_path)
            if cfg.ACTIVE_LEARNING.SAMPLING_FN == "typiclust_rp":
                activeSet, new_uSet, clusters = al_obj.sample_from_uSet(model, lSet, uSet, all_data, dataset_info=dataset_info)
            else:
                activeSet, new_uSet = al_obj.sample_from_uSet(model, lSet, uSet, all_data, dataset_info=dataset_info)
            ...
            msg = f"Update training set size {len(train_dataset)}"
            print(msg);logger.info(msg)
            print("================================\n\n")
            logger.info("================================\n\n")

            if not skip_training and not cfg.ACTIVE_LEARNING.FINE_TUNE: 
                msg = "load init state dict, train from scratch"
                model.load_state_dict({"module."+k : v for k, v in init_model_state_dict.items()})
            elif not skip_training and cfg.ACTIVE_LEARNING.FINE_TUNE:
                msg = "load best model from previous al iter"
                model_state_dict = torch.load(best_model_save_path, map_location=device)
        ...
        if all_sampled_set.size > 0:
            ...
            if use_embedding:
                z, tb, pd_min, pd_max = calculate_param_for_plot_embedding(embedding_path, dataset_info)

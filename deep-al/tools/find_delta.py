...
# FOLDS = 4
fold_idx_row = np.zeros((train_val_df.shape[0]), dtype="int32")
fold_idx_row[train_val_df["patient_code"] == "A3104"] = 0
fold_idx_row[train_val_df["patient_code"] == "C2104"] = 0

fold_idx_row[train_val_df["patient_code"] == "A2111"] = 1
fold_idx_row[train_val_df["patient_code"] == "C2205"] = 1

fold_idx_row[train_val_df["patient_code"] == "A3102"] = 2
fold_idx_row[train_val_df["patient_code"] == "C2202"] = 2

fold_idx_row[train_val_df["patient_code"] == "A1115"] = 3
fold_idx_row[train_val_df["patient_code"] == "C2201"] = 3
train_val_df.loc[:, "fold_idx"] = fold_idx_row
...
if __name__ == "__main__":
    ...
    args_parser.add_argument("--seed", type=int, required=True)
    args_parser.add_argument("--fold_idx", type=int, required=True)
    args = args_parser.parse_args()
    seed = args.seed
    fold_idx = args.fold_idx

    neyes = (train_val_df["fold_idx"] != fold_idx).sum()
    nclasses = neyes
    features = ds_utils.load_features(f"blink_fold{fold_idx}", seed=seed, train=True)
    km = KMeans(nclasses)
    ...
    print(delta_list[first_drop_below_thres])

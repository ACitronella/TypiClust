...
from utils.train_utils_pe import simclr_pe_train
from utils.utils_wo_target import fill_memory_bank
from utils.utils_wo_target_pe import fill_memory_bank_pe
...
def main():
    ...
    if "blink" in p['train_db_name']:
        train_dataset, sampler = get_train_dataset(p, train_transforms, to_augmented_dataset=True,
    ...
    train_dataloader = get_train_dataloader(p, train_dataset, sampler)
    # val_dataloader = get_val_dataloader(p, val_dataset)
    print('Dataset contains {}/{} train/val samples'.format(len(train_dataset), len(val_dataset)))

    # sampled_idx = []
    # for batch in train_dataloader:
    #     sampled_idx.extend([train_dataset.get_patient_code_and_frame_from_idx(idx)[0].name for idx in batch["meta"]["index"]])
    # import pandas as pd
    # print(pd.Series(sampled_idx).value_counts())
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        if p['setup'] == 'simclr_pe':
            simclr_pe_train(train_dataloader, model, criterion, optimizer, epoch)
        else:
